...
void ReadSonicDistance()
{
  // this function shuts off the motors if the ultrasonic sensors read a distance too short to stop from
  digitalWrite(US_1_TX_PIN, LOW);
  digitalWrite(US_2_TX_PIN, LOW);
  digitalWrite(US_3_TX_PIN, LOW);
  delayMicroseconds(2);
  digitalWrite(US_1_RX_PIN, HIGH);
  digitalWrite(US_2_RX_PIN, HIGH);
  digitalWrite(US_3_RX_PIN, HIGH);
  delayMicroseconds(10);
  digitalWrite(US_1_TX_PIN, LOW);
  digitalWrite(US_2_TX_PIN, LOW);
  digitalWrite(US_3_TX_PIN, LOW);

  float duration1 = pulseIn(US_1_RX_PIN, HIGH);
  float duration2 = pulseIn(US_1_RX_PIN, HIGH);
  float duration3 = pulseIn(US_1_RX_PIN, HIGH);

  float distance1 = (duration1*.0343)/2;
  float distance2 = (duration2*.0343)/2;
  float distance3 = (duration3*.0343)/2;
...
void setup()
{
  ...
  //Ultrasonic Pins
  pinMode(US_1_TX_PIN, OUTPUT);
  pinMode(US_1_RX_PIN, INPUT);
  pinMode(US_2_TX_PIN, OUTPUT);
  pinMode(US_2_RX_PIN, INPUT);
  pinMode(US_3_TX_PIN, OUTPUT);
  pinMode(US_3_RX_PIN, INPUT);
